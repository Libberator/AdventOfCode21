...
public class Program
{
    ...
    private static void Benchmark(dynamic puzzle)  // change to <int>/<long> as needed
    {
        var iterations = 1000;
        using (new Watch($"Part 1 x {iterations}")) {
            for (int i = 0; i < iterations; i++) {
                _ = puzzle.SolvePart1();
            }
        }

        using (new Watch($"Part 2 x {iterations}")) {
            for (int i = 0; i < iterations; i++) {
                _ = puzzle.SolvePart2();
            }
        }
    ...
    private static dynamic? GetPuzzle(int day) => day switch
    {
